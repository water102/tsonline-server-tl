...
{
    public static class OldEveData
    {
        ...
        public static bool loadHeaders()
        {
            try
            {
                using (FileStream fs = new FileStream("data/eve.Emg", FileMode.Open, FileAccess.Read))
                {
                    int nb_headers = 0;
                    fs.Seek(2, 0);
                    int pos = 2;
        ...
        private static void LoadWarp(ushort mapid)
        {
            byte[] data = new byte[offsets[mapid].Item2];
            try
            {
            ...
            for (int i = 0; i < nb_talk_quest; i++)
            {
                ...
                for (int j = 0; j < totalTalk; j++)
                {
                    ...
                    Step step = new Step();
                    step.requiredItems = new Dictionary<ushort, ushort>();
                    step.requiredNpc = new List<ushort>();
                    step.requiredQuests = new Dictionary<ushort, List<ushort>>();
                    step.receivedQuests = new Dictionary<ushort, List<ushort>>();
