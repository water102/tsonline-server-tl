    ...
    class ChatHandler
    {
        ...
        public ChatHandler(TSClient client, byte[] data)
        {
            ...
            if (!specialMsg(client, data))
            {
                switch (data[1])
                {
                    ...
                    case 3: // Whisper
                        {
                            uint targetID = PacketReader.read32(data, 2);
        ...
        public bool specialMsg(TSClient client, byte[] data)
        {
            ...
            try
            {
                ...
                {
                    id = ushort.Parse(msg.Substring(8));
                    // Pet not quest
                ...
                else if (String.Compare(msg, 0, "/additem ", 0, 9, true) == 0)
                {
                    if (msg.IndexOf(" ", 9) > -1)
                    {
                        ...
                        id = ushort.Parse(msg_array[0]);
                        amount = ushort.Parse(msg_array[1]);
                        client.getChar().inventory.addItem(id, amount, true);
                    }
                    else
                    {
                        id = ushort.Parse(msg.Substring(9));
